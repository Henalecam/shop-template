	...
	const columns = useMemo<GridColDef<Template>[]>(
		() => [
			...
			{ field: 'createdAt', headerName: 'Criado em', width: 160 },
			{
				...
				headerName: 'Slug',
				width: 140,
				valueGetter: (params: GridValueGetterParams<Template>) => params.row.slug || '-',
			},
			{
				field: 'actions',
				headerName: 'Ações',
				...
				sortable: false,
				renderCell: (params) => {
					const slug = (params.row as Template).slug;
					if (!slug) return null;
					return (
						<Link className="text-brand-400 hover:underline" href={`/templates/${slug}`}>Abrir</Link>
					);
				},
			},
		],
